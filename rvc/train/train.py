...
def train_and_evaluate(rank, epoch, hps, nets, optims, scaler, loaders, writers, cache):
    ...
    if rank == 0:
        print(f"{hps.name} | epoch={epoch} | steps={global_step} | {epoch_recorder.record()} | loss_disc={loss_disc:.3f} | loss_gen={loss_gen:.3f} | loss_fm={loss_fm:.3f} | loss_mel={loss_mel:.3f} | loss_kl={loss_kl:.3f}")
