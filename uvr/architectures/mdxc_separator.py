...
class MDXCSeparator(CommonSeparator):
    ...
    def __init__(self, common_config, arch_config):
        ...
        self.override_model_segment_size = arch_config.get(
        ...
        self.logger.debug(
            f"MDXC arch params: override_model_segment_size={self.override_model_segment_size}, pitch_shift={self.pitch_shift}"
        )
        ...
        self.is_vocal_main_target = (
            True
            if self.model_data_cfgdict.training.target_instrument == "Vocals"
            else False
        )
        ...
        try:
            if self.is_roformer:
                ...
                if "num_bands" in self.model_data_cfgdict.model:
                    ...
                    raise ValueError(
                        "Unknown Roformer model type in the configuration."
                    )
    ...
    def separate(self, audio_file_path):
        ...
        if not isinstance(self.primary_source, np.ndarray):
            self.logger.debug(
                f"Normalizing primary source for primary stem {self.primary_stem_name}..."
            )
        ...
        if not isinstance(self.secondary_source, np.ndarray):
            self.logger.debug(
                f"Normalizing secondary source for secondary stem {self.secondary_stem_name}..."
            )
    ...
    def demix(self, mix: np.ndarray) -> dict:
        ...
        if self.is_roformer:
            ...
            if self.override_model_segment_size:
                ...
                self.logger.debug(
                    f"Using model default segment size: {mdx_segment_size}"
                )
            ...
            chunk_size = self.model_data_cfgdict.audio.hop_length * (
                mdx_segment_size - 1
            )
            ...
            window = torch.tensor(
                signal.windows.hamming(chunk_size), dtype=torch.float32
            )
            ...
            with torch.no_grad():
                req_shape = (
                    len(self.model_data_cfgdict.training.instruments),
                ) + tuple(mix.shape)
        ...
        if num_stems > 1 or self.is_vocal_main_target:
            self.logger.debug(
                "Number of stems is greater than 1 or vocals are main target, detaching individual sources and correcting pitch if necessary..."
            )
            ...
            if self.is_vocal_main_target:
                self.logger.debug(
                    "Vocals are main target, detaching vocals and matching array shapes if necessary..."
                )
                if sources["Vocals"].shape[1] != orig_mix.shape[1]:
                    sources["Vocals"] = spec_utils.match_array_shapes(
                        sources["Vocals"], orig_mix
                    )
